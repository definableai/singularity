...
class Settings(BaseSettings):
    ...
    database_url: str = Field("", description="Postgres URL (postgresql+asyncpg://...). Empty = db features disabled (loudly).")
    db_pooler: str = Field("none", description="none | pgbouncer — pgbouncer flips asyncpg prepared-statement caches off.")
    redis_url: str = Field("redis://localhost:6379/0", description="Redis connection URL.")
    cors_origins: list[str] = Field([], description='JSON list of allowed CORS origins, e.g. ["https://app.example.com"].')
    forwarded_allow_ips: str = Field("127.0.0.1", description="Comma-separated IPs/CIDRs trusted for X-Forwarded-* headers.")
    request_timeout_s: int = Field(15, description="Per-request deadline (s). statement_timeout <= this < SHUTDOWN_GRACE_S.")
    shutdown_grace_s: int = Field(20, description="SIGTERM drain window (s).")
    ...
    trace_code_roots: list[str] = Field(
        ["src/services", "src/models", "src/tasks", "src/scripts"],
        description="Directories whose code the tracer records. Add new top-level src dirs here.",
    )
    auth_providers: list[str] = Field([], description='Auth chain: shipped names ("jwt", "api_key") or dotted BYO class paths.')
    jwt_secret: str = Field("", description="JWT HS* shared secret (secret mode).")
    jwt_jwks_url: str = Field("", description="JWKS URL (https only). Setting this enables JWKS mode; iss+aud become mandatory.")
    jwt_algorithms: list[str] = Field(["HS256"], description="Allowed JWT algorithms (allowlist checked before signature).")
    jwt_issuer: str = Field("", description="Expected iss claim (mandatory in JWKS mode).")
    jwt_audience: str = Field("", description="Expected aud claim (mandatory in JWKS mode).")
    api_key_cache_ttl: int = Field(60, description="API-key cache TTL (s); revoked keys stay valid up to this per process.")
    auth_dev_principal: str = Field("", description="Dev only: credential-less requests get this principal id. Boot error outside dev.")
    obs_retention_days: int = Field(7, description="records partitions older than this are dropped.")
    dashboard_token: str = Field("", description="Required to reach /__obs outside dev (Bearer or ?token=).")
