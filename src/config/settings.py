...
class Settings(BaseSettings):
    ...
    environment: Environment = Field(
        description="dev | staging | prod. No default — boot fails without it."
    )
    database_url: str = Field(
        default="",
        description="Postgres URL (postgresql+asyncpg://...). Empty = db features disabled (loudly).",
    )
    db_pooler: str = Field(
        default="none",
        description="none | pgbouncer — pgbouncer flips asyncpg prepared-statement caches off.",
    )
    redis_url: str = Field(default="redis://localhost:6379/0", description="Redis connection URL.")
    cors_origins: list[str] = Field(
        default=[],
        description='JSON list of allowed CORS origins, e.g. ["https://app.example.com"].',
    )
    forwarded_allow_ips: str = Field(
        default="127.0.0.1",
        description="Comma-separated IPs/CIDRs trusted for X-Forwarded-* headers.",
    )
    request_timeout_s: int = Field(
        default=15,
        description="Per-request deadline (s). statement_timeout <= this < SHUTDOWN_GRACE_S.",
    )
    shutdown_grace_s: int = Field(default=20, description="SIGTERM drain window (s).")
    obs_transports: list[str] | None = Field(
        default=None,
        description="Obs transports (jsonl|stdout|null). Unset = derived: dev=jsonl, else stdout.",
    )
    obs_redact_fields: list[str] = Field(
        default=[], description="Extra field names to redact in obs events (extends built-ins)."
    )
    obs_flush_timeout_s: int = Field(
        default=5, description="Bound on the final obs flush at shutdown (s)."
    )
    trace_mode: str = Field(
        default="",
        description="dev | on_error | off. Unset = derived from ENVIRONMENT. dev mode only allowed when ENVIRONMENT=dev.",
    )
    trace_slow_ms: int = Field(
        default=1000, description="on_error mode also emits journeys slower than this (ms)."
    )
    trace_sample_rate: float = Field(
        default=0.1, description="on_error mode: fraction of requests captured at T2."
    )
    ...
    auth_providers: list[str] = Field(
        default=[],
        description='Auth chain: shipped names ("jwt", "api_key") or dotted BYO class paths.',
    )
    jwt_secret: str = Field(default="", description="JWT HS* shared secret (secret mode).")
    jwt_jwks_url: str = Field(
        default="",
        description="JWKS URL (https only). Setting this enables JWKS mode; iss+aud become mandatory.",
    )
    jwt_algorithms: list[str] = Field(
        default=["HS256"],
        description="Allowed JWT algorithms (allowlist checked before signature).",
    )
    jwt_issuer: str = Field(default="", description="Expected iss claim (mandatory in JWKS mode).")
    jwt_audience: str = Field(
        default="", description="Expected aud claim (mandatory in JWKS mode)."
    )
    api_key_cache_ttl: int = Field(
        default=60,
        description="API-key cache TTL (s); revoked keys stay valid up to this per process.",
    )
    auth_dev_principal: str = Field(
        default="",
        description="Dev only: credential-less requests get this principal id. Boot error outside dev.",
    )
    obs_retention_days: int = Field(
        default=7, description="records partitions older than this are dropped."
    )
    dashboard_token: str = Field(
        default="", description="Required to reach /__obs outside dev (Bearer or ?token=)."
    )
    dataviews_db_url: str = Field(
        default="",
        description="Read-only role URL for Observatory data views (sg db grant-readonly). Empty = data views disabled.",
    )
    dashboard_users_sql: str = Field(
        default="",
        description="SELECT returning id, email, name, created_at for Observatory's Users view. Empty = principals from records.",
    )
